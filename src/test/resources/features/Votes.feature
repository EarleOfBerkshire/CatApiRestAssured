...
@votes
  ...
  Scenario: Create a vote (requires API key)
    ...
    When I send a POST request to "/votes" with image_id and vote value
    Then the response status code should be 201
    And the response should indicate a successful vote

  Scenario: Delete a vote (requires API key)
    Given I have a valid vote ID "some_vote_id"
    When I send a DELETE request to "/votes/{vote_id}"
    Then the response status code should be 200
  ...
  Scenario: Create vote with invalid vote value
    GivenI have a valid image ID "some_image_id"
    And I have a vote value of 99
    When I send a POST request to "/votes" with image_id and vote value
    Then the response status code should be 400

  Scenario: Delete non-existent vote
    Given I have an invalid vote ID "invalid_vote"
